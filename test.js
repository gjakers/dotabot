...
client.on("ready", async () => {
	console.log("Start...");
	client.user.setActivity('Shame Simulator 3');
...
client.ws.on('INTERACTION_CREATE', async (interaction) => {
	...
	if (interaction.type == 2)
	{
		...
		if(command == 'dota') {		
			...
			while(matches.length){
				embeds.push(recentEmbed(matches.splice(0,5)));
			}
	...
	if (interaction.type == 3) // BUTTON
	{
		...
		const message = await client.api.webhooks(client.user.id, interaction.token).messages('@original').patch({data: {}});
		console.log(message);

		// 	{data: {type: 4,
		// 			data: {content: "Recent matches for **" + playerName + "**",
		// 				   embeds: embeds[1],
		// 			}
		// 		}
		// 	}
		...
		//client.api.interactions(interaction.id, interaction.token).callback.messages.patch()
